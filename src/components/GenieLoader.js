...
export default function GenieLoader({ size = 80, animating = true, style }) {
  // Animated values
  ...
  // Repeats every ~2.5s: bounce+shake → pause → bounce+shake → pause
  function createBounce() {
    return Animated.sequence([
      ...
      Animated.parallel([
        // Vertical hop sequence (up then down)
        Animated.sequence([
          Animated.parallel([
        ...
        // Horizontal rattle runs while airborne (~180ms, fits inside the 290ms hop)
        createMidAirShake(),
      ]),
      // Landing squash
      Animated.timing(genieScaleY, {
  ...
  function startAnimation() {
    ...
    const introBounce = Animated.sequence([
      ...
      // Launch + fall + mid-air rattle (all parallel)
      Animated.parallel([
        Animated.sequence([
          Animated.parallel([
            Animated.timing(genieBounceY, {
        ...
        // Horizontal rattle while airborne
        createMidAirShake(),
      ]),
      // Landing squash
      Animated.timing(genieScaleY, {
    ...
    introRef.current.start(() => {
      ...
      // Phase 3: Looping animations

      // Self-accelerating orbit
      startOrbitCycle(5000);

      // Periodic genie bounce (every ~2.5s)
      startBounceLoop();
