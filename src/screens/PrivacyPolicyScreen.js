...
• To manage your subscription tier, token balance, and premium feature access based on your purchase history
...
• To record when the App Store rating request is shown to you, along with the number of AI generations you have completed at that point, so we can measure prompt effectiveness, avoid showing the prompt too often, and improve our notification timing. This data is tied to your account and is not shared with third parties.
...
• Full name and email address when you register with email
...
• Optional onboarding intake — if you fill out the optional fields during the onboarding flow (first name, where you currently shop for home design, how you heard about us), your answers are stored locally on your device. The first name pre-fills the signup form and, if you complete registration, becomes part of your account profile (covered by the "Full name" bullet above). The "where you shop" and "how you heard about us" answers remain on your device only and are never uploaded to our servers. You may clear them at any time by reinstalling the App.

Identifiers (collected automatically)
• User ID: A unique account identifier generated by Supabase when you register. This ID is stored in our database and linked to all of your content and activity in the App.
• Device ID (Push Token): Your device's Expo push notification token, stored in our database and linked to your account when you enable push notifications. This token is used solely to deliver notifications to your device.

Room Photos and AI Content (provided by you)
...
• AI-generated room wishes and design images saved to your account

Product Interaction and Usage Data (collected automatically)
• Products you browse, tap, add to cart, or follow affiliate links to purchase
...
• We store your current subscription tier (Free or Pro), token balance, and a record of in-app purchases (subscriptions and token packs) in our database. This is used solely to manage your access to premium features and track your token balance. We do NOT collect or store credit card numbers, bank account details, or Apple transaction IDs. All financial transactions are processed exclusively by Apple through the App Store.
...
• Black Forest Labs GmbH — Alternative AI image generation model (flux-kontext-pro, flux-pro). Same data handling as above. Privacy policy: blackforestlabs.ai/privacy
• fal.ai (Fal, Inc.) — Alternative AI inference provider used for image generation tasks. Receives image prompts; returns generated images. Privacy policy: fal.ai/privacy

AI Prompt Expansion, Vision, and Product Matching
• Anthropic, PBC — Used in two places: (1) to expand short user-typed text prompts (for example, "modern bedroom") into more detailed scene descriptions before those prompts are sent to the image-generation model; (2) to analyze AI-generated room images and identify furniture and décor items for product recommendations. For prompt expansion, Anthropic receives only your text prompt — not your room photo. For product matching, Anthropic receives the AI-generated design image (not your original room photo) and returns structured item identification data. Anthropic does not use submitted prompts or images for model training under their API terms. Privacy policy: anthropic.com/privacy
